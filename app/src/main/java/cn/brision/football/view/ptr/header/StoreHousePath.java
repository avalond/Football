package cn.brision.football.view.ptr.header;
...
public class StoreHousePath {
    ...
    private static final SparseArray<float[]> SPARSE_ARRAY;

    static {
        SPARSE_ARRAY = new SparseArray<float[]>();
        ...
        for (int i = 0; i < floats.length; i++) {
            SPARSE_ARRAY.append(i + 65, floats[i]);
        }
        // a - z
        for (int i = 0; i < floats.length; i++) {
            SPARSE_ARRAY.append(i + 65 + 32, floats[i]);
        }
        // 0 - 9
        for (int i = 0; i < numbers.length; i++) {
            SPARSE_ARRAY.append(i + 48, numbers[i]);
        }
        // blank
        addChar(' ', new float[]{});
        // -
    ...
    public static ArrayList<float[]> getPath(String str, float scale, int gapBetweenLetter) {
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            int key = SPARSE_ARRAY.indexOfKey(pos);
            if (key == -1) {
                continue;
            }
            float[] points = SPARSE_ARRAY.get(pos);
