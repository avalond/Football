package cn.brision.football.view.ptr;
...
import cn.brision.football.view.ptr.util.PtrCLog;

/**
 * This layout view for "Pull to Refresh(Ptr)" support all of the view, you can contain everything you want.
 * support: pull to refresh / release to refresh / auto refresh /
 * keep header view while refreshing / hide header view while refreshing
 * It defines {@link PtrUIHandler},
 * which allows you customize the UI easily.
 */
public class PtrFrameLayout extends ViewGroup {
    ...
    protected static final String LOG_TAG = "ptr-frame-" + ++sId;
    // auto refresh status
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        if (mDisableWhenHorizontalMove || !isEnabled() || mContent == null || mHeaderView == null) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (DEBUG) {
                    ...
                    PtrCLog.v(LOG_TAG, "ACTION_MOVE: offsetY:%s, currentPos: %s, moveUp: %s, " +
                                    "canMoveUp: %s, moveDown: %s: canMoveDown: %s", offsetY, mPtrIndicator.getCurrentPosY(),
                            moveUp, canMoveUp, moveDown, canMoveDown);
                }
    ...
    private boolean tryToPerformRefresh() {
        ...
        if ((mPtrIndicator.isOverOffsetToKeepHeaderWhileLoading()
                && isAutoRefresh()) || mPtrIndicator.isOverOffsetToRefresh()) {
     ...
     * The UI will perform complete at once or after a delay, depends on
     * the time elapsed is greater then {@link #mLoadingMinTime} or not.
     */
    ...
    public void autoRefresh() {
        autoRefresh(true, mDurationToCloseHeader, 0);
    }

    public void autoRefresh(int tag) {
        autoRefresh(true, mDurationToCloseHeader, tag);
    }

    public void autoRefresh(boolean atOnce) {
        autoRefresh(atOnce, mDurationToCloseHeader, 0);
    }
    ...
    public void autoRefresh(boolean atOnce, int duration, int tag) {
        ...
        if (tag == 0) {
            mScrollChecker.tryToScrollTo(mPtrIndicator.getOffsetToKeepHeaderWhileLoading(), duration);
        } else {
            mScrollChecker.tryToScrollTo(tag, duration);
        }
        if (atOnce) {
            mStatus = PTR_STATUS_LOADING;
            performRefresh();
