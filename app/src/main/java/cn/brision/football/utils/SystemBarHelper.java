...
public class SystemBarHelper
{
     ...
     * 增加View的高度以及paddingTop,增加的值为状态栏高度.一般是在沉浸式全屏给ToolBar用的
     */
    public static void setHeightAndPadding1(Context context, View view)
    {

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT)
        {
            ViewGroup.LayoutParams lp = view.getLayoutParams();
            lp.height += getStatusBarHeight(context)/2;//增高
//            view.setPadding(view.getPaddingLeft(), view.getPaddingTop() + getStatusBarHeight(context),
//                    view.getPaddingRight(), view.getPaddingBottom());
        }
    }
     ...
     * 增加View的高度以及paddingTop,增加的值为状态栏高度.一般是在沉浸式全屏给ToolBar用的
     */
    public static void setHeightAndPadding2(Context context, View view)
    {
...
//                    view.getPaddingRight(), view.getPaddingBottom());
     ...
     * Android4.4以上的沉浸式全屏模式
     * 注:
     * 1,当横评状态的时候将statusbar进行颜色透明设置
     * 2,当横评的时候将上移的高度进行去除
     *
     * @param activity Activity对象
     */
    public static void immersiveStatusBarReduction(Activity activity)
    {

        immersiveStatusBarReduction(activity, (float) 0.0);
    }
     ...
     * @param alpha    透明栏透明度[0.0-1.0]
     */
    public static void immersiveStatusBarReduction(Activity activity, @FloatRange(from = 0.0, to = 1.0) float alpha)
    {
        immersiveStatusBarReduction(activity.getWindow(), alpha);
    }
     ...
     * @param alpha  透明栏透明度[0.0-1.0]
     */
    public static void immersiveStatusBarReduction(Window window, @FloatRange(from = 0.0, to = 1.0) float alpha)
    {
